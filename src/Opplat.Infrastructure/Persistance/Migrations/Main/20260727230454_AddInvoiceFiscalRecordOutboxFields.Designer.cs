// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;
using Opplat.Infrastructure.Persistance.Data;

#nullable disable

namespace Opplat.Infrastructure.Persistance.Migrations.Main
{
    [DbContext(typeof(OpplatDbContext))]
    [Migration("20260727230454_AddInvoiceFiscalRecordOutboxFields")]
    partial class AddInvoiceFiscalRecordOutboxFields
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "10.0.5")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("AnnotationOrderDetail", b =>
                {
                    b.Property<Guid>("AnnotationsId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("OrderDetailsSaleDetailId")
                        .HasColumnType("uuid");

                    b.HasKey("AnnotationsId", "OrderDetailsSaleDetailId");

                    b.HasIndex("OrderDetailsSaleDetailId");

                    b.ToTable("AnnotationOrderDetail");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.BaseEntity", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasDefaultValueSql("gen_random_uuid()");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CreatedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<DateTime>("ModifiedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("ModifiedBy")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.HasKey("Id");

                    b.ToTable((string)null);

                    b.UseTpcMappingStrategy();
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Inventory.ProductClassification", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("Description")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.ToTable("ProductClassifications");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Inventory.ProductGroup", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<int>("ClassificationId")
                        .HasColumnType("integer");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("ClassificationId");

                    b.ToTable("ProductGroups");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Inventory.ProductIncomeDetail", b =>
                {
                    b.Property<Guid>("ProductId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("UnitId")
                        .HasColumnType("uuid");

                    b.Property<decimal>("Quantity")
                        .HasColumnType("numeric");

                    b.Property<decimal>("TotalAmount")
                        .HasColumnType("numeric");

                    b.HasKey("ProductId", "UnitId");

                    b.ToTable("ProductIncomeDetail");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Inventory.ProductInventory", b =>
                {
                    b.Property<Guid>("ProductId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("StorageId")
                        .HasColumnType("uuid");

                    b.Property<decimal>("Quantity")
                        .HasColumnType("numeric");

                    b.Property<DateTime>("UpdatedOn")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("User")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("ProductId", "StorageId");

                    b.HasIndex("StorageId");

                    b.ToTable("Inventories");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Inventory.VoucherDetail", b =>
                {
                    b.Property<Guid>("ProductId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("VoucherId")
                        .HasColumnType("uuid");

                    b.Property<decimal>("Quantity")
                        .HasColumnType("numeric");

                    b.HasKey("ProductId", "VoucherId");

                    b.HasIndex("VoucherId");

                    b.ToTable("VoucherDetail");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Sales.AddedTopping", b =>
                {
                    b.Property<Guid>("ToppingId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("SaleDetailId")
                        .HasColumnType("uuid");

                    b.Property<decimal>("Amount")
                        .HasColumnType("numeric");

                    b.Property<int>("Quantity")
                        .HasColumnType("integer");

                    b.HasKey("ToppingId", "SaleDetailId");

                    b.HasIndex("SaleDetailId");

                    b.ToTable("AddedTopping");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Sales.CostTab", b =>
                {
                    b.Property<Guid>("ProductId")
                        .HasColumnType("uuid");

                    b.Property<decimal>("ExpectedRate")
                        .HasColumnType("numeric");

                    b.Property<decimal>("FixedCost")
                        .HasColumnType("numeric");

                    b.Property<decimal>("PlannedCost")
                        .HasColumnType("numeric");

                    b.Property<string>("Preparation")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Presentation")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("ProductId");

                    b.ToTable("CostTabs");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Sales.CostTabDetail", b =>
                {
                    b.Property<Guid>("ProductForSaleId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("ProductId")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("CostTabProductId")
                        .HasColumnType("uuid");

                    b.Property<decimal>("FixedCost")
                        .HasColumnType("numeric");

                    b.Property<decimal>("Quantity")
                        .HasColumnType("numeric");

                    b.Property<string>("Unit")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<decimal>("VariableCost")
                        .HasColumnType("numeric");

                    b.HasKey("ProductForSaleId", "ProductId");

                    b.HasIndex("CostTabProductId");

                    b.HasIndex("ProductId");

                    b.ToTable("CostTabDetail");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Sales.OrderDetail", b =>
                {
                    b.Property<Guid>("SaleDetailId")
                        .HasColumnType("uuid");

                    b.Property<string>("CustomerType")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Observations")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<int>("Position")
                        .HasColumnType("integer");

                    b.HasKey("SaleDetailId");

                    b.ToTable("OrderDetail");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.UnitOfMeasurement", b =>
                {
                    b.Property<string>("Abbreviation")
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<decimal>("CovertionFactor")
                        .HasColumnType("numeric");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("UnitType")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Abbreviation");

                    b.ToTable("UnitOfMeasurement");
                });

            modelBuilder.Entity("Opplat.Domain.Models.License", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("Application")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateTime>("ExpirationDate")
                        .HasColumnType("timestamp with time zone");

                    b.Property<byte[]>("Hash")
                        .IsRequired()
                        .HasColumnType("bytea");

                    b.Property<string>("Subscriber")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.ToTable("Licenses");
                });

            modelBuilder.Entity("Opplat.Domain.Models.Notification", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("Link")
                        .HasColumnType("text");

                    b.Property<string>("Module")
                        .HasColumnType("text");

                    b.Property<string>("Text")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.ToTable("Notifications");
                });

            modelBuilder.Entity("Opplat.Domain.Models.UserNotification", b =>
                {
                    b.Property<int>("NotificationId")
                        .HasColumnType("integer");

                    b.Property<string>("UserId")
                        .HasColumnType("text");

                    b.Property<bool>("IsRead")
                        .HasColumnType("boolean");

                    b.HasKey("NotificationId", "UserId");

                    b.ToTable("UserNotifications");
                });

            modelBuilder.Entity("ProductForSaleProductTag", b =>
                {
                    b.Property<Guid>("OnProductsId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("TagsId")
                        .HasColumnType("uuid");

                    b.HasKey("OnProductsId", "TagsId");

                    b.HasIndex("TagsId");

                    b.ToTable("ProductForSaleProductTag");
                });

            modelBuilder.Entity("ProductForSaleTopping", b =>
                {
                    b.Property<Guid>("AvailableToppingsId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("OnProductsId")
                        .HasColumnType("uuid");

                    b.HasKey("AvailableToppingsId", "OnProductsId");

                    b.HasIndex("OnProductsId");

                    b.ToTable("ProductForSaleTopping");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Inventory.Product", b =>
                {
                    b.HasBaseType("Opplat.Domain.Entities.BaseEntity");

                    b.Property<bool>("Active")
                        .HasColumnType("boolean");

                    b.Property<decimal>("ContainerQuantity")
                        .HasColumnType("numeric");

                    b.Property<decimal>("Cost")
                        .HasColumnType("numeric");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<decimal>("FixedValue")
                        .HasColumnType("numeric");

                    b.Property<int>("GroupId")
                        .HasColumnType("integer");

                    b.Property<bool>("ItsInventoriable")
                        .HasColumnType("boolean");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<decimal>("ShrinkRatio")
                        .HasColumnType("numeric");

                    b.Property<string>("Unit")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasIndex("GroupId");

                    b.ToTable("Products");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Inventory.ProductMovement", b =>
                {
                    b.HasBaseType("Opplat.Domain.Entities.BaseEntity");

                    b.Property<decimal>("Cost")
                        .HasColumnType("numeric");

                    b.Property<DateTime>("Date")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("Observations")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<Guid>("ProductId")
                        .HasColumnType("uuid");

                    b.Property<decimal>("Quantity")
                        .HasColumnType("numeric");

                    b.Property<Guid>("StorageId")
                        .HasColumnType("uuid");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("User")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasIndex("ProductId");

                    b.HasIndex("StorageId");

                    b.ToTable("ProductMovements");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Inventory.ReleaseVoucher", b =>
                {
                    b.HasBaseType("Opplat.Domain.Entities.BaseEntity");

                    b.Property<string>("AuthorizedBy")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateTime>("Date")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<Guid?>("DestinationId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("OriginId")
                        .HasColumnType("uuid");

                    b.HasIndex("DestinationId");

                    b.HasIndex("OriginId");

                    b.ToTable("ReleaseVoucher");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Inventory.Storage", b =>
                {
                    b.HasBaseType("Opplat.Domain.Entities.BaseEntity");

                    b.Property<string>("Code")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<bool>("IsCostCenter")
                        .HasColumnType("boolean");

                    b.ToTable("Storages");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Inventory.TangibleFixedAsset", b =>
                {
                    b.HasBaseType("Opplat.Domain.Entities.BaseEntity");

                    b.Property<decimal>("CurrentValue")
                        .HasColumnType("numeric");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateTime?>("DischargeDate")
                        .HasColumnType("timestamp with time zone");

                    b.Property<bool>("Discharged")
                        .HasColumnType("boolean");

                    b.Property<DateTime>("EntryDate")
                        .HasColumnType("timestamp with time zone");

                    b.Property<decimal>("InitialValue")
                        .HasColumnType("numeric");

                    b.ToTable("TangibleFixedAsset");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Invoicing.Customer", b =>
                {
                    b.HasBaseType("Opplat.Domain.Entities.BaseEntity");

                    b.Property<string>("Address")
                        .HasColumnType("text");

                    b.Property<string>("Country")
                        .HasColumnType("text");

                    b.Property<string>("Email")
                        .HasColumnType("text");

                    b.Property<bool>("IsFinalConsumer")
                        .HasColumnType("boolean");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("TaxId")
                        .HasColumnType("text");

                    b.ToTable("Customers");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Invoicing.Invoice", b =>
                {
                    b.HasBaseType("Opplat.Domain.Entities.BaseEntity");

                    b.Property<string>("Currency")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<Guid?>("CustomerId")
                        .HasColumnType("uuid");

                    b.Property<string>("FullNumber")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<int>("InvoiceType")
                        .HasColumnType("integer");

                    b.Property<DateTime>("IssueDate")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("Notes")
                        .HasColumnType("text");

                    b.Property<int>("Number")
                        .HasColumnType("integer");

                    b.Property<string>("PaymentMethod")
                        .HasColumnType("text");

                    b.Property<Guid?>("SaleId")
                        .HasColumnType("uuid");

                    b.Property<string>("Series")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<int>("Status")
                        .HasColumnType("integer");

                    b.Property<decimal>("Subtotal")
                        .HasColumnType("numeric");

                    b.Property<decimal>("TotalAmount")
                        .HasColumnType("numeric");

                    b.HasIndex("CustomerId");

                    b.HasIndex("SaleId");

                    b.HasIndex("Series", "Number")
                        .IsUnique();

                    b.ToTable("Invoices");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Invoicing.InvoiceCounter", b =>
                {
                    b.HasBaseType("Opplat.Domain.Entities.BaseEntity");

                    b.Property<int>("LastNumber")
                        .HasColumnType("integer");

                    b.Property<string>("Series")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<int>("Year")
                        .HasColumnType("integer");

                    b.HasIndex("Series", "Year")
                        .IsUnique();

                    b.ToTable("InvoiceCounters");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Invoicing.InvoiceFiscalRecord", b =>
                {
                    b.HasBaseType("Opplat.Domain.Entities.BaseEntity");

                    b.Property<string>("AeatCsv")
                        .HasColumnType("text");

                    b.Property<string>("AeatResponseRaw")
                        .HasColumnType("text");

                    b.Property<DateTime?>("AeatSubmittedAtUtc")
                        .HasColumnType("timestamp with time zone");

                    b.Property<DateTime>("GeneratedAtUtc")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("HashInput")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<Guid>("InvoiceId")
                        .HasColumnType("uuid");

                    b.Property<string>("LastErrorMessage")
                        .HasColumnType("text");

                    b.Property<DateTime?>("NextRetryAtUtc")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("PreviousRecordHash")
                        .HasColumnType("text");

                    b.Property<string>("QrCodePayload")
                        .HasColumnType("text");

                    b.Property<string>("RecordHash")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<int>("RetryCount")
                        .HasColumnType("integer");

                    b.Property<string>("SignatureValue")
                        .HasColumnType("text");

                    b.Property<string>("SoftwareLicenseId")
                        .HasColumnType("text");

                    b.Property<string>("SoftwareName")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("SoftwareVersion")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<int>("SubmissionMode")
                        .HasColumnType("integer");

                    b.Property<int>("SubmissionStatus")
                        .HasColumnType("integer");

                    b.HasIndex("InvoiceId")
                        .IsUnique();

                    b.HasIndex("SubmissionStatus", "NextRetryAtUtc");

                    b.ToTable("InvoiceFiscalRecords");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Invoicing.InvoiceLine", b =>
                {
                    b.HasBaseType("Opplat.Domain.Entities.BaseEntity");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<decimal>("DiscountAmount")
                        .HasColumnType("numeric");

                    b.Property<Guid>("InvoiceId")
                        .HasColumnType("uuid");

                    b.Property<decimal>("LineTotal")
                        .HasColumnType("numeric");

                    b.Property<Guid?>("ProductId")
                        .HasColumnType("uuid");

                    b.Property<decimal>("Quantity")
                        .HasColumnType("numeric");

                    b.Property<decimal>("TaxRate")
                        .HasColumnType("numeric");

                    b.Property<decimal>("UnitPrice")
                        .HasColumnType("numeric");

                    b.HasIndex("InvoiceId");

                    b.ToTable("InvoiceLines");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Invoicing.InvoiceTaxBreakdown", b =>
                {
                    b.HasBaseType("Opplat.Domain.Entities.BaseEntity");

                    b.Property<Guid>("InvoiceId")
                        .HasColumnType("uuid");

                    b.Property<decimal>("Rate")
                        .HasColumnType("numeric");

                    b.Property<decimal>("TaxAmount")
                        .HasColumnType("numeric");

                    b.Property<int>("TaxType")
                        .HasColumnType("integer");

                    b.Property<decimal>("TaxableBase")
                        .HasColumnType("numeric");

                    b.HasIndex("InvoiceId");

                    b.ToTable("InvoiceTaxBreakdowns");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Invoicing.TenantFiscalSettings", b =>
                {
                    b.HasBaseType("Opplat.Domain.Entities.BaseEntity");

                    b.Property<string>("BusinessSector")
                        .HasColumnType("text");

                    b.Property<string>("Country")
                        .HasColumnType("text");

                    b.Property<string>("DefaultSeries")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("FiscalAddress")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<int>("InvoicingMode")
                        .HasColumnType("integer");

                    b.Property<string>("LegalName")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<decimal>("SimplifiedInvoiceThreshold")
                        .HasColumnType("numeric");

                    b.Property<string>("SoftwareLicenseId")
                        .HasColumnType("text");

                    b.Property<string>("TaxId")
                        .IsRequired()
                        .HasColumnType("text");

                    b.ToTable("TenantFiscalSettings");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Sales.Annotation", b =>
                {
                    b.HasBaseType("Opplat.Domain.Entities.BaseEntity");

                    b.Property<string>("Acronym")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("text");

                    b.ToTable("Annotations");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Sales.Discount", b =>
                {
                    b.HasBaseType("Opplat.Domain.Entities.BaseEntity");

                    b.Property<decimal>("Amount")
                        .HasColumnType("numeric");

                    b.Property<bool>("IsPercent")
                        .HasColumnType("boolean");

                    b.Property<Guid>("SaleId")
                        .HasColumnType("uuid");

                    b.Property<string>("User")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasIndex("SaleId");

                    b.ToTable("Discount");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Sales.ProductForSale", b =>
                {
                    b.HasBaseType("Opplat.Domain.Entities.BaseEntity");

                    b.Property<bool>("Active")
                        .HasColumnType("boolean");

                    b.Property<string>("Code")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<decimal>("Price")
                        .HasColumnType("numeric");

                    b.ToTable("ProductsForSale");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Sales.ProductTag", b =>
                {
                    b.HasBaseType("Opplat.Domain.Entities.BaseEntity");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text");

                    b.ToTable("ProductTags");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Sales.Sale", b =>
                {
                    b.HasBaseType("Opplat.Domain.Entities.BaseEntity");

                    b.Property<DateTime>("Date")
                        .HasColumnType("timestamp with time zone");

                    b.Property<decimal>("TotalAmount")
                        .HasColumnType("numeric");

                    b.ToTable("Sales");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Sales.SaleDetail", b =>
                {
                    b.HasBaseType("Opplat.Domain.Entities.BaseEntity");

                    b.Property<decimal>("Amount")
                        .HasColumnType("numeric");

                    b.Property<Guid>("ProductId")
                        .HasColumnType("uuid");

                    b.Property<int>("Quatity")
                        .HasColumnType("integer");

                    b.Property<Guid>("SaleId")
                        .HasColumnType("uuid");

                    b.HasIndex("ProductId");

                    b.HasIndex("SaleId");

                    b.ToTable("SaleDetail");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Sales.Topping", b =>
                {
                    b.HasBaseType("Opplat.Domain.Entities.BaseEntity");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text");

                    b.ToTable("Toppings");
                });

            modelBuilder.Entity("Opplat.Domain.Models.User", b =>
                {
                    b.HasBaseType("Opplat.Domain.Entities.BaseEntity");

                    b.Property<string>("Email")
                        .HasColumnType("text");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean");

                    b.Property<string>("LastName")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("UserName")
                        .HasColumnType("text");

                    b.ToTable("Users");
                });

            modelBuilder.Entity("AnnotationOrderDetail", b =>
                {
                    b.HasOne("Opplat.Domain.Entities.Sales.Annotation", null)
                        .WithMany()
                        .HasForeignKey("AnnotationsId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Opplat.Domain.Entities.Sales.OrderDetail", null)
                        .WithMany()
                        .HasForeignKey("OrderDetailsSaleDetailId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Inventory.ProductGroup", b =>
                {
                    b.HasOne("Opplat.Domain.Entities.Inventory.ProductClassification", "Classification")
                        .WithMany("Groups")
                        .HasForeignKey("ClassificationId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Classification");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Inventory.ProductInventory", b =>
                {
                    b.HasOne("Opplat.Domain.Entities.Inventory.Product", "Product")
                        .WithMany()
                        .HasForeignKey("ProductId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Opplat.Domain.Entities.Inventory.Storage", "Storage")
                        .WithMany()
                        .HasForeignKey("StorageId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Product");

                    b.Navigation("Storage");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Inventory.VoucherDetail", b =>
                {
                    b.HasOne("Opplat.Domain.Entities.Inventory.Product", "Product")
                        .WithMany()
                        .HasForeignKey("ProductId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Opplat.Domain.Entities.Inventory.ReleaseVoucher", "Voucher")
                        .WithMany("Products")
                        .HasForeignKey("VoucherId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Product");

                    b.Navigation("Voucher");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Sales.AddedTopping", b =>
                {
                    b.HasOne("Opplat.Domain.Entities.Sales.SaleDetail", "SaleDetail")
                        .WithMany("Toppings")
                        .HasForeignKey("SaleDetailId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Opplat.Domain.Entities.Sales.Topping", "Topping")
                        .WithMany()
                        .HasForeignKey("ToppingId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("SaleDetail");

                    b.Navigation("Topping");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Sales.CostTab", b =>
                {
                    b.HasOne("Opplat.Domain.Entities.Sales.ProductForSale", "Product")
                        .WithOne()
                        .HasForeignKey("Opplat.Domain.Entities.Sales.CostTab", "ProductId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Product");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Sales.CostTabDetail", b =>
                {
                    b.HasOne("Opplat.Domain.Entities.Sales.CostTab", null)
                        .WithMany("Details")
                        .HasForeignKey("CostTabProductId");

                    b.HasOne("Opplat.Domain.Entities.Sales.ProductForSale", "ProductForSale")
                        .WithMany()
                        .HasForeignKey("ProductForSaleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Opplat.Domain.Entities.Inventory.Product", "Product")
                        .WithMany()
                        .HasForeignKey("ProductId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Product");

                    b.Navigation("ProductForSale");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Sales.OrderDetail", b =>
                {
                    b.HasOne("Opplat.Domain.Entities.Sales.SaleDetail", "SaleDetail")
                        .WithOne("OrderDetail")
                        .HasForeignKey("Opplat.Domain.Entities.Sales.OrderDetail", "SaleDetailId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("SaleDetail");
                });

            modelBuilder.Entity("Opplat.Domain.Models.UserNotification", b =>
                {
                    b.HasOne("Opplat.Domain.Models.Notification", "Notification")
                        .WithMany("UserNotification")
                        .HasForeignKey("NotificationId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Notification");
                });

            modelBuilder.Entity("ProductForSaleProductTag", b =>
                {
                    b.HasOne("Opplat.Domain.Entities.Sales.ProductForSale", null)
                        .WithMany()
                        .HasForeignKey("OnProductsId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Opplat.Domain.Entities.Sales.ProductTag", null)
                        .WithMany()
                        .HasForeignKey("TagsId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("ProductForSaleTopping", b =>
                {
                    b.HasOne("Opplat.Domain.Entities.Sales.Topping", null)
                        .WithMany()
                        .HasForeignKey("AvailableToppingsId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Opplat.Domain.Entities.Sales.ProductForSale", null)
                        .WithMany()
                        .HasForeignKey("OnProductsId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Inventory.Product", b =>
                {
                    b.HasOne("Opplat.Domain.Entities.Inventory.ProductGroup", "Group")
                        .WithMany()
                        .HasForeignKey("GroupId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Group");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Inventory.ProductMovement", b =>
                {
                    b.HasOne("Opplat.Domain.Entities.Inventory.Product", "Product")
                        .WithMany()
                        .HasForeignKey("ProductId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Opplat.Domain.Entities.Inventory.Storage", "Storage")
                        .WithMany()
                        .HasForeignKey("StorageId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Product");

                    b.Navigation("Storage");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Inventory.ReleaseVoucher", b =>
                {
                    b.HasOne("Opplat.Domain.Entities.Inventory.Storage", "Destination")
                        .WithMany()
                        .HasForeignKey("DestinationId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.HasOne("Opplat.Domain.Entities.Inventory.Storage", "Origin")
                        .WithMany()
                        .HasForeignKey("OriginId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Destination");

                    b.Navigation("Origin");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Invoicing.Invoice", b =>
                {
                    b.HasOne("Opplat.Domain.Entities.Invoicing.Customer", "Customer")
                        .WithMany()
                        .HasForeignKey("CustomerId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("Opplat.Domain.Entities.Sales.Sale", "Sale")
                        .WithMany()
                        .HasForeignKey("SaleId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.OwnsOne("Opplat.Domain.Entities.Invoicing.CustomerSnapshot", "CustomerSnapshot", b1 =>
                        {
                            b1.Property<Guid>("InvoiceId")
                                .ValueGeneratedOnAdd()
                                .HasColumnType("uuid");

                            b1.Property<string>("Address")
                                .HasColumnType("text");

                            b1.Property<string>("Country")
                                .HasColumnType("text");

                            b1.Property<bool>("IsFinalConsumer")
                                .HasColumnType("boolean");

                            b1.Property<string>("Name")
                                .IsRequired()
                                .HasColumnType("text");

                            b1.Property<string>("TaxId")
                                .HasColumnType("text");

                            b1.HasKey("InvoiceId");

                            b1.ToTable("Invoices");

                            b1.WithOwner()
                                .HasForeignKey("InvoiceId");
                        });

                    b.Navigation("Customer");

                    b.Navigation("CustomerSnapshot")
                        .IsRequired();

                    b.Navigation("Sale");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Invoicing.InvoiceFiscalRecord", b =>
                {
                    b.HasOne("Opplat.Domain.Entities.Invoicing.Invoice", "Invoice")
                        .WithOne("FiscalRecord")
                        .HasForeignKey("Opplat.Domain.Entities.Invoicing.InvoiceFiscalRecord", "InvoiceId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Invoice");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Invoicing.InvoiceLine", b =>
                {
                    b.HasOne("Opplat.Domain.Entities.Invoicing.Invoice", "Invoice")
                        .WithMany("Lines")
                        .HasForeignKey("InvoiceId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Invoice");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Invoicing.InvoiceTaxBreakdown", b =>
                {
                    b.HasOne("Opplat.Domain.Entities.Invoicing.Invoice", "Invoice")
                        .WithMany("TaxBreakdowns")
                        .HasForeignKey("InvoiceId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Invoice");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Sales.Discount", b =>
                {
                    b.HasOne("Opplat.Domain.Entities.Sales.Sale", "Sale")
                        .WithMany("Discounts")
                        .HasForeignKey("SaleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Sale");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Sales.SaleDetail", b =>
                {
                    b.HasOne("Opplat.Domain.Entities.Sales.ProductForSale", "Product")
                        .WithMany()
                        .HasForeignKey("ProductId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Opplat.Domain.Entities.Sales.Sale", "Sale")
                        .WithMany("Products")
                        .HasForeignKey("SaleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Product");

                    b.Navigation("Sale");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Inventory.ProductClassification", b =>
                {
                    b.Navigation("Groups");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Sales.CostTab", b =>
                {
                    b.Navigation("Details");
                });

            modelBuilder.Entity("Opplat.Domain.Models.Notification", b =>
                {
                    b.Navigation("UserNotification");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Inventory.ReleaseVoucher", b =>
                {
                    b.Navigation("Products");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Invoicing.Invoice", b =>
                {
                    b.Navigation("FiscalRecord");

                    b.Navigation("Lines");

                    b.Navigation("TaxBreakdowns");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Sales.Sale", b =>
                {
                    b.Navigation("Discounts");

                    b.Navigation("Products");
                });

            modelBuilder.Entity("Opplat.Domain.Entities.Sales.SaleDetail", b =>
                {
                    b.Navigation("OrderDetail");

                    b.Navigation("Toppings");
                });
#pragma warning restore 612, 618
        }
    }
}
