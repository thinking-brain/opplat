...
export const getStoredTenantIdentifier = (): string | null =>
  typeof window === 'undefined' ? null : window.localStorage.getItem('opplat_tenant');
...
export const persistTenantIdentifier = (tenantIdentifier: string | null): void => {
  ...
  if (tenantIdentifier) {
    window.localStorage.setItem('opplat_tenant', tenantIdentifier);
    return;
  }

  window.localStorage.removeItem('opplat_tenant');
};

export const buildAppUser = (oidcUser: OidcUser): User => {
  const claims = getMergedClaims(oidcUser);
  const username =
    asString(readClaim(claims, ['preferred_username', 'unique_name', 'name', 'email'])) ?? 'usuario';
