  ...
  <v-container grid-list-xl fluid>
    <v-card>
      ...
      <v-data-table
        ...
        class="elevation-1"
        @getProveedoresFromApi="getProveedoresFromApi"
        :footer-props="{
...
<script>
import api from "@/api";
import AddEdit from "@/components/taller/proveedores/AddEdit.vue";
...
export default {
  ...
  data() {
    return {
      ...
      cardTitle: " Listado de Proveedores",
      entity: "Proveedores",
      totalItems: 0,
    };
  },
  ...
  watch: {},
  created() {
    this.getProveedoresFromApi();
  },
  methods: {
    getProveedoresFromApi() {
      this.loading = true;
      const url = api.getUrl("taller", "Proveedores");
      this.axios
        .get(url)
        .then((response) => {
