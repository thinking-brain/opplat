    ...
    <v-data-table :headers="headers" :items="contratos" :search="search" class="elevation-1 pa-5">
        ...
        <template v-slot:top>
            <v-toolbar flat color="white">
                ...
                <v-spacer></v-spacer>
                <v-text-field v-model="search" append-icon="mdi-magnify" label="Buscar" single-line hide-details clearable dense></v-text-field>
                <v-spacer></v-spacer>

                <!-- Todos los Contratos -->
                <v-badge :content="cantContratos" :value="cantContratos" color="primary" overlap class="mt-4">
                    <template v-slot:badge>
                        <span v-if="enTiempo > 0">{{ cantContratos }}</span>
                    </template>
                    <v-tooltip top color="primary">
                        <template v-slot:activator="{ on }">
                            <v-icon medium v-on="on" color="primary" @click="getContratosFromApi()">mdi-file-document-box-multiple-outline</v-icon>
                        </template>
                ...
                <v-badge :content="enTiempo" :value="enTiempo" color="green" overlap class="mt-4 ml-4">
                    ...
                    <v-tooltip top color="green">
                        <template v-slot:activator="{ on }">
                            <v-icon medium v-on="on" color="green" @click="filtro(contratoTiempo)">mdi-file-document-box-multiple-outline</v-icon>
                        </template>
                ...
                <v-badge :content="proxVencer" :value="proxVencer" color="orange" overlap class="mt-4 ml-4">
                    ...
                    <v-tooltip top color="orange">
                        <template v-slot:activator="{ on }">
                            <v-icon medium v-on="on" color="orange" @click="filtro(contratosProxVencer)">mdi-file-document-box-multiple-outline</v-icon>
                        </template>
                ...
                <v-badge :content="casiVenc" :value="casiVenc" color="deep-orange" overlap class="mt-4 ml-4">
                    <template v-slot:badge>
                        <span v-if="casiVenc > 0">{{ casiVenc }}</span>
                    </template>
                    <v-tooltip top color="deep-orange">
                        <template v-slot:activator="{ on }">
                            <v-icon medium v-on="on" color="deep-orange" @click="filtro(contratosCasiVenc)">mdi-file-document-box-multiple-outline</v-icon>
                        </template>
                ...
                <v-badge :content="vencidos" :value="vencidos" color="red" overlap class="mt-4 ml-4">
                    ...
                    <v-tooltip top color="red">
                        <template v-slot:activator="{ on }">
                            <v-icon medium v-on="on" color="red" @click="filtro(contratosVenc)">mdi-file-document-box-multiple-outline</v-icon>
                        </template>
        ...
        <template v-slot:item.action="{ item }">
            <v-row>
                <v-tooltip top color="primary">
                    <template v-slot:activator="{ on }">
                        <v-btn class="v-btn v-btn--depressed v-btn--fab v-btn--flat v-btn--icon v-btn--outlined v-btn--round theme--dark v-size--small primary--text" small v-on="on" @click="newSuplemento(item)" slot="activator" v-if="
                ...
                <v-tooltip top color="teal">
                    <template v-slot:activator="{ on }">
                        <v-btn class="v-btn v-btn--depressed v-btn--fab v-btn--flat v-btn--icon v-btn--outlined v-btn--round theme--dark v-size--small teal--text" small v-on="on" @click="getDetalles(item)">
                ...
                <v-tooltip top color="black">
                    <template v-slot:activator="{ on }">
                        <v-btn class="v-btn v-btn--depressed v-btn--fab v-btn--flat v-btn--icon v-btn--outlined v-btn--round theme--dark v-size--small secondary--text" small v-on="on" @click="download(item)">
...
export default {
    data: () => ({
        ...
        headers: [{
            ...
            {
                text: "Entidad",
                value: "entidad[0].nombre",
            },
            {
                text: "Sector",
                value: "entidad[0].sectorNombre",
            },
            {
                text: "Vence",
                value: "contVence",
    ...
    methods: {
        ...
        newSuplemento(item) {
            ...
            for (
                let index = 0; index < contrato.especialistasExternos.length; index++
            ) {
