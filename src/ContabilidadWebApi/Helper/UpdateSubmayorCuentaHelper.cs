...
namespace ContabilidadWebApi.Helper
{
    public class UpdateSubmayorCuentaHelper
    {
        ...
        public void LlenarSubMayor2()
        {
            ...
            //var data = submayorCuenta.Any(s=>s.Mes >= 5) ? _vcontext.Set<OptCuentaPeriodoOK>().Include(s => s.IdperiodoNavigation).Where(s => s.Idperiodo >= periodoUltimo.Idperiodo).ToList() : _vcontext.Set<OptCuentaPeriodoOK>().Include(s => s.IdperiodoNavigation).Where(s => s.Idperiodo >= 43).ToList();
            var data = _vcontext.Set<OptCuentaPeriodoOK>().Include(s => s.IdperiodoNavigation).Where(s => s.Idperiodo >= 43).ToList();
            ...
            foreach (var item in data.OrderBy(s => s.Idperiodo))
            {
                ...
                s.Mes == item.IdperiodoNavigation.Inicio.Month &&
                s.IdPaseDeCuenta == item.Idpasecuenta
                ))
                {
                    ...
                    if (item.Clavecuenta.StartsWith("912"))
                    {
                        var mov = new SubMayorCuenta
                        {
                            ...
                            IdPaseDeCuenta = item.Idpasecuenta
                        };
                    ...
                    {
                        if (item.IdperiodoNavigation.Inicio.Month == 4 && item.IdperiodoNavigation.Inicio.Year == 2019)
                        {
                            var x = 0;
                        }
                        var mov = new SubMayorCuenta
