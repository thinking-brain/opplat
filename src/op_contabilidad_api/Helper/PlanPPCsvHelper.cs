...
using op_contabilidad_api.Helpers;
...
namespace op_contabilidad_api.Helper
{
    ...
    /// </summary>
    public class PlanPPCsvHelper
    {

        private readonly DbContext _context;
        private readonly VersatDbContext _vcontext;

        public PlanPPCsvHelper( ApiDbContext _context, VersatDbContext _vcontext)
        {
            this._context = _context;
            this._vcontext = _vcontext;

        }
        public PlanPPCsvHelper( DbContext _context, VersatDbContext _vcontext)
        {
            this._context = _context;
            this._vcontext = _vcontext;
        ...
        private void readingElements(CsvReader reader, string año)
        {
            ...
            while (reader.Read())
            {
                ...
                PlanPronosticoProductivo plan = new PlanPronosticoProductivo();
                plan.Año = año;

                string subcode = line[0];

                ////Llena el plan
                //if (!ReadElement(subcode, plan)) continue;
                ReadMonthValues(months, plan);
                plan.Fecha = DateTime.Now;
                var claves = line[1].Split('-');
                var CUC = _vcontext.ConCuentamoneda.Where(s => s.Idcuenta == Convert.ToInt32(claves[0].Trim()) && s.Idmoneda == 2);
                var CUP = _vcontext.ConCuentamoneda.Where(s => s.Idcuenta == Convert.ToInt32(claves[1].Trim()) && s.Idmoneda == 1);
                var ctas = _context.Set<>
                //var CUC = _vcontext.ConCuentamoneda.SingleOrDefault(s => s.Idcuenta == Convert.ToInt32(claves[0]));
                plan.CuentaMN = CUP.Count() > 0 ? claves[1] : claves[0];
                plan.CuentaCUC = CUC.Count() > 0 ? claves[0] : claves[1];
                if (!_context.Set<PlanPronosticoProductivo>().Any(s=>s.Año == plan.Año && s.CuentaCUC == plan.CuentaCUC && s.CuentaMN == plan.CuentaMN))
                {
                    _context.Add(plan);
                    try
                    {
                ...
                else
                {
                    ...
                    var planSelect = _context.Set<PlanPronosticoProductivo>().SingleOrDefault(s => s.Año == plan.Año && s.CuentaCUC == plan.CuentaCUC && s.CuentaMN == plan.CuentaMN);
        ...
        /// <summary>
        /// Determina si es un Elemento
        /// </summary>
        /// <param name="record">Regsitro</param>
        /// <param name="code">Codigo del Elemento.</param>
        ...
        ///// <returns></returns>
        //private bool ReadElement(string subcode, PlanPronosticoProductivo plan)
        //{
        //    //var data = DatosPlanGI.Datos().SingleOrDefault(s => s.Dato.Trim().ToUpper().Equals(subcode.Trim().ToUpper()));
        //    plan.Elemento = FixText.Fix(subcode).ToUpper().TrimEnd();
        //    //plan.ElementoValor = Convert.ToString(subcode);
        ...
        private void ReadMonthValues(List<string> values, PlanPronosticoProductivo plan)
        {
        ...
        //    {
        //        PlanPronosticoProductivo emptyPlan = new PlanPronosticoProductivo();
