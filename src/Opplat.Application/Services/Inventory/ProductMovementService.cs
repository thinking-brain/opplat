...
public interface IProductMovementService : IService<ProductMovement, string>
{
    Task<ServiceResponse<ProductMovement>> CreateMovement(Guid productId, Guid storageId, decimal quantity, string unit, DateTime? date
         , MovementType movementType, string user, string observations);
    Task<ServiceResponse<ProductMovement>> GetByStorage(string storageId);
}

public class ProductMovementService(IMovementsRepository repo, IMovementTypeService movementTypeService,
    IProductRepository productRepo, IStorageRepository storageRepo, ILogger<ProductMovement> logger) : BaseService<ProductMovement, string>(repo, logger), IProductMovementService
{
    readonly IMovementTypeService _movementTypeService = movementTypeService;
    readonly IProductRepository _productRepo = productRepo;
    readonly IStorageRepository _storageRepo = storageRepo;
        ...
        , MovementType movementType, string user, string observations = "")
    {
        ...
        decimal adjustedAmount = quantity * (UnitOfMeasurementHelper.GetByAbbreviation(product.Unit).CovertionFactor / unit.CovertionFactor);
        ...
        int factor = _movementTypeService.GetFactor(movementType);
        var repo = _repo as IMovementsRepository;
        var result = await repo.Create(movement, factor, unit.Abbreviation);
        if (result.IsOk)
        {
            return new MovementResult { IsValid = true };
        }
        return new MovementResult { IsValid = false, Unit = unit.Abbreviation, ConvertedQuantity = adjustedAmount, Quantity = quantity };
    }

    public async Task<ServiceResponse<ProductMovement>> GetByStorage(string storageId)
    {
        var id = new Guid(storageId);
        var repo = _repo as IMovementsRepository;
        var list = await repo.GetByStorage(id);
        var result = new ServiceResponse<ProductMovement>
        {
    ...
    public async Task<ServiceResponse<ProductMovement>> CreateMovement(Guid productId, Guid storageId, decimal quantity, string unit, DateTime? date, MovementType movementType, string user, string observations)
    {
        if (date == null)
        {
            date = DateTime.UtcNow;
        }
        var prod = await _productRepo.Find(productId);
        var storage = await _storageRepo.Find(storageId);
        var unitOM = UnitOfMeasurementHelper.GetByAbbreviation(unit);
        var result = await CreateMovement(prod, storage, quantity, unitOM, date.Value, movementType, user, observations);
        if (result.IsValid)
        {
            return new ServiceResponse<ProductMovement>
            {
                Status = ServiceStatus.Ok,
        ...
        return new ServiceResponse<ProductMovement>
        {
            ...
            Message = $"You need {result.ConvertedQuantity} {result.Unit} to make the movement"
        };
    }
