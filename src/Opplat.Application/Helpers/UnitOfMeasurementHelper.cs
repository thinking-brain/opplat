...
namespace Opplat.Application.Helpers;
...
public class UnitOfMeasurementHelper
{
    ...
    {
        var result = _units.FirstOrDefault(u => u.Abbreviation.Equals(abbreviation, StringComparison.CurrentCultureIgnoreCase));
        return result;
    }

    public static UnitOfMeasurement? GetByName(string name)
    {
        var result = _units.FirstOrDefault(u => u.Name.Equals(name, StringComparison.CurrentCultureIgnoreCase));
        return result;
    }
}
