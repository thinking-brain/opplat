...
- My domain: `test/Opplat.Api.Main.Test/` — backend test project
- Test run command: `dotnet test .\test\Opplat.Api.Main.Test\Opplat.Api.Main.Test.csproj --no-build --logger "console;verbosity=minimal"`
...
- Current local port contract is inconsistent: Aspire runs `admin-api` on `http://localhost:8084`, but `src\opplat-react\src\runtimeConfig.ts` still falls back to `http://localhost:5160`, which is the standalone `Opplat.Api.Admin` launchSettings port.
