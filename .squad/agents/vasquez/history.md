...
- 2026-03-25 — User registration frontend implemented in `opplat-react`. Pattern: `adminPublicAxiosClient` (plain `axios.create` with `baseURL: appConfig.adminApiUrl`) exported from `axiosClient.ts` for public/unauthenticated endpoints — no OIDC token injection, no tenant scoping. `adminApiUrl` config key added to `runtimeConfig.ts` (`VITE_ADMIN_API_URL`, default `http://localhost:5160`). `registerUser` function in `auth.api.ts` maps `name`→`firstName`, `lastName`→`lastName` for the AdminApi payload. `RegisterPage.tsx` follows LoginPage Card+Container pattern. Route `/register` added as a public route (outside `ProtectedRoute`) in `App.tsx`.
...
- 2026-03-23 — For Opplat SPAs in Aspire, use `AddViteApp(...)` plus a dedicated `dev:aspire` script, then make `vite.config.ts` honor `PORT` and suppress `open` when `OPPLAT_RUNNING_IN_ASPIRE=true` so AppHost gets stable hot-reload ports without extra browser tabs.
