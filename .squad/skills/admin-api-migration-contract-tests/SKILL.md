...
- If the host uses a shared application-registration helper (for example `AddOpplatApplication(...)`) instead of calling `AddMediatR(...)` directly, pin both seams: the host must call the helper and the helper must still register MediatR from the intended assemblies.
