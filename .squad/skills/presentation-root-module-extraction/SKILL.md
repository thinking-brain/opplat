...
### Remove MVC only after explicit endpoint mapping is complete
Before deleting `AddControllers()` / `MapControllers()`, make `Program.cs` enumerate every surviving minimal endpoint extension explicitly (for example admin + account + tenant feature modules). That final checklist prevents accidentally dropping a still-needed surface while ensuring archived controllers can never become live again through leftover MVC registration.
